...
- [ ] **All Daily Backups Successful**
  - Command: `aws s3 ls s3://huston-housing-backups-prod/full/ | grep -E "$(date -d '7 days ago' +%Y%m%d|date -d '6 days ago' +%Y%m%d|date -d '5 days ago' +%Y%m%d|date -d '4 days ago' +%Y%m%d|date -d '3 days ago' +%Y%m%d|date -d '2 days ago' +%Y%m%d|date +%Y%m%d)"`
  - Expected: 7 backup folders, one per day
  - Action: If missing, review failed backup logs

- [ ] **No Critical Errors Last 7 Days**
  - Command: `grep -i error /var/log/huston-housing/backup-*.log | grep -i critical | wc -l`
  - Expected: 0 critical errors
  - Action: Address any critical errors found

- [ ] **All Incremental Backups Completed**
  - Command: `aws s3 ls s3://huston-housing-backups-prod/incremental/ | tail -28`
  - Expected: 28 incremental backups (4 per day × 7 days)
  - Action: If missing any, verify they completed

- [ ] **WAL Archiving Continuous**
  - Command: `psql -U huston-housing -c "SELECT archived_count, failed_count FROM pg_stat_archiver;" | tail -1`
...
- [ ] **S3 Storage Usage**
  - Command: `aws s3api list-objects-v2 --bucket huston-housing-backups-prod --recursive --query 'Contents[].Size' | jq 'add / 1099511627776'`
  - Expected: < 2 TB total
  - Action if > 2 TB: Review retention policies, clean old backups

- [ ] **S3 Access Verification**
  - Command: `aws s3 ls s3://huston-housing-backups-prod/ --recursive | wc -l`
  - Expected: Thousands of files accessible
  - Action if error: Verify AWS credentials, check permissions

- [ ] **Backup Encryption Verified**
  - Command: `aws s3api head-object --bucket huston-housing-backups-prod --key full/$(date +%Y%m%d)_000000/base.tar.gz | jq .ServerSideEncryption`
  - Expected: "AES256"
  - Action if missing: Verify S3 encryption policy enabled

- [ ] **S3 Versioning Active**
  - Command: `aws s3api get-bucket-versioning --bucket huston-housing-backups-prod | jq .Status`
  - Expected: "Enabled"
...
- [ ] **Database Size Tracking**
  - Command: `psql -U huston-housing -c "SELECT pg_size_pretty(pg_database_size('huston-housing'));"`
  - Expected: Consistent with previous week (within ±10%)
  - Action if significantly larger: Investigate data growth

- [ ] **Table Count Verification**
  - Command: `psql -U huston-housing -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public';"`
  - Expected: Should match documentation (currently ~30 tables)
  - Action: If count changes unexpectedly, investigate

- [ ] **Index Status**
  - Command: `psql -U huston-housing -c "SELECT schemaname, tablename, indexname FROM pg_indexes WHERE schemaname = 'public' ORDER BY tablename;"`
  - Expected: No missing or duplicate indexes
  - Action: If changed, review with application team

- [ ] **Unused Indexes**
  - Command: `psql -U huston-housing -c "SELECT schemaname, tablename, indexname FROM pg_stat_user_indexes WHERE idx_scan = 0;"`
...
- [ ] **Download Latest Backup**
  - Command: `BACKUP=$(aws s3 ls s3://huston-housing-backups-prod/full/ | tail -1 | awk '{print $4}') && aws s3 cp s3://huston-housing-backups-prod/full/${BACKUP} /tmp/`
  - Expected: Backup downloaded successfully (usually 1-2 GB)
  - Time: 10-30 minutes depending on network

- [ ] **Create Test Database**
  - Command: `psql -U postgres -c "DROP DATABASE IF EXISTS huston-housing_weekly_test; CREATE DATABASE huston-housing_weekly_test;"`
  - Expected: Test database created successfully
  - Verify: `psql -U postgres -l | grep weekly_test`

- [ ] **Run Restore Test**
  - Command: `/opt/huston-housing/scripts/test-restore.sh /tmp/${BACKUP}`
...
- [ ] **Verify Restored Data Integrity**
  - Commands:
    ```bash
    psql -U postgres -d huston-housing_weekly_test -c "SELECT COUNT(*) FROM users WHERE deleted_at IS NULL;"
    psql -U postgres -d huston-housing_weekly_test -c "SELECT COUNT(*) FROM properties WHERE deleted_at IS NULL;"
    psql -U postgres -d huston-housing_weekly_test -c "SELECT COUNT(*) FROM rental_agreements;"
    ```
...
- [ ] **Test Critical Queries**
  - Commands:
    ```bash
    psql -U postgres -d huston-housing_weekly_test -c "SELECT COUNT(*) FROM rental_agreements WHERE status = 'active';"
    psql -U postgres -d huston-housing_weekly_test -c "SELECT COUNT(*) FROM escrow WHERE amount > 0;"
    psql -U postgres -d huston-housing_weekly_test -c "SELECT COUNT(*) FROM disputes WHERE resolution_status IS NULL;"
    ```
  - Expected: All queries return results
  - Action if errors: Check for schema or permission issues

- [ ] **Clean Up Test Database**
  - Command: `psql -U postgres -c "DROP DATABASE huston-housing_weekly_test;"`
  - Expected: Database dropped successfully
  - Cleanup: `rm -f /tmp/huston-housing_*.sql.gz`
...
- [ ] **Snapshot Exists and is Recent**
  - Command: `aws s3 ls s3://huston-housing-backups-prod/snapshot/ | tail -3`
  - Expected: Snapshot from most recent Sunday
  - Action if missing: Create snapshot manually: `/opt/huston-housing/scripts/backup-snapshot.sh`

- [ ] **Snapshot Size Reasonable**
  - Command: `aws s3 ls s3://huston-housing-backups-prod/snapshot/ | tail -1 | awk '{print $5}'`
  - Expected: 1.5-3 GB (similar to full backup)
  - Action if too small: Investigate

- [ ] **Snapshot Uploaded to Glacier**
  - Command: `aws s3api head-object --bucket huston-housing-backups-prod --key snapshot/$(ls -t /tmp | grep -m1 snapshot | cut -d. -f1) | jq .StorageClass`
  - Expected: "GLACIER"
  - Action if different: Check S3 lifecycle policy

- [ ] **Snapshot Metadata Valid**
  - Command: `aws s3 cp s3://huston-housing-backups-prod/snapshot/metadata_$(date -d 'last sunday' +%Y%m%d).json - | jq .`
...
- [ ] **Select Backup for Monthly Drill**
  ...
  - Command: `aws s3 ls s3://huston-housing-backups-prod/full/ | head -1`
...
- [ ] **Backup Performance Analysis**
  - Review backup durations: `grep 'completed' /var/log/huston-housing/backup-*.log | tail -7`
...
3. Review test script logs: `tail -50 /var/log/huston-housing/test-restore.log`
4. **Do NOT proceed** if restore fails - investigate immediately

### If Snapshot Missing or Corrupted

1. Check S3 directory: `aws s3 ls s3://huston-housing-backups-prod/snapshot/`
2. Create new snapshot: `/opt/huston-housing/scripts/backup-snapshot.sh`
3. Verify upload: `aws s3 ls s3://huston-housing-backups-prod/snapshot/ | tail -3`
4. Document incident

### If Metrics Show Degradation

1. Identify trend: Which metric is declining?
2. Check database: `psql -U huston-housing -c "SELECT pg_size_pretty(pg_database_size('huston-housing'));"`
3. Check resources: `top`, `df -h`, `iostat -x`
4. Review logs: `tail -100 /var/log/huston-housing/backup-full.log`
